...
const adminUIPatch = `
          {adminViewMode === 'admin' ? (
            <div className="bg-[#1a1a1a] border border-[#262626] rounded-xl p-8 flex flex-col items-center justify-center text-center space-y-6 h-[50vh] animate-in fade-in slide-in-from-bottom-2 duration-300">
...
// we need to close `</>` at the end of the p-6 div, which is right before `</main>` or just before the modals.
// Let's close it right before `        {/* Modal 2: Expanded Table Breakdown */}` if that's easier.
const modal2Index = afterContent.indexOf('{/* Modal 2: Expanded Table Breakdown */}');
const innerContent = afterContent.substring(0, modal2Index);
const remainingContent = afterContent.substring(modal2Index);
